...
void matchLeftRightOptFlow(const pangolin::ManagedImage<uint8_t>& currImgL,
  ...
  for (int i = 0; i < currPointsTemp.size(); i++) {
    ...
    Eigen::Vector2f diff((currPointsTemp[i] - currPointsL[i]).x,
                         (currPointsTemp[i] - currPointsL[i]).y);
    Eigen::Vector2d tempy(round(currPointsL[i].x), round(currPointsL[i].y));
  ...
  for (int i = 0; i < currPointsL.size(); i++) {
    if (checkyboi % 5 == 0) {
      cv::Point src((int)currPointsL[i].x, (int)currPointsL[i].y);
      cv::Point trgt(currPointsR[i].x + imageR.cols, currPointsR[i].y);
